...
- **Интегрировать** внешние сервисы: **Git (для источников и результатов)**, OpenRouter (LLM), Supabase (PostgreSQL), **vsegpt.ru (векторизация)**, Qdrant.
- **Создать** воспроизводимый и конфигурируемый конвейер данных на **Node.js**, включающий **обработчик webhook\'ов** и **логику пуша результатов в Git**.
...
6.  **Модуль Основной Обработки (LLM + Supabase):** (Как в v1.3) Обрабатывает чанки, извлекает информацию через LLM, сохраняет в Supabase.
7.  **Модуль Векторизации (vsegpt.ru + Qdrant):** (Как в v1.3) Векторизует данные из Supabase, загружает в Qdrant.
8.  **Публикация Результатов:** После успешного завершения, скрипт клонирует/обновляет **Git-репозиторий Приложения**, копирует туда чанковые `.md` файлы из хранилища Coolify, коммитит и пушит изменения.
9.  **Оркестрация и Деплоймент:** Весь стек (Node.js приложение с HTTP-сервером, Supabase/PostgreSQL, Qdrant) управляется через **Coolify**.
...
graph TD
    ...
    subgraph "Основной Автоматизированный Конвейер"
        ...
        subgraph Pipeline ["Пайплайн Обработки (для module_id)"]
            ...
            P1 -- Чанки MD в Хранилище --> P2(6. LLM + Supabase);
...
- **4.1.5. Доступ (Запись Результатов):** Система должна иметь настроенный доступ (SSH-ключ или токен) для выполнения `git push` в **Git-репозиторий Приложения**.
...
- **4.3.2. Сохранение в Supabase (PostgreSQL):** (Без изменений)
- **4.3.3. Векторизация и Загрузка в Qdrant:** (Без изменений)
- **4.3.4. Публикация Результатов в Git:**
    ...
    - **Работа с Git:**
        - Клонировать или обновить локальную копию **репозитория Приложения** во временной директории внутри контейнера.
        - Скопировать/переместить все `.md` файлы чанков для обработанного `module_id` в целевую папку репозитория Приложения (например, `output/processed_results/<module_id>/<source_file_name>/`).
        - Выполнить `git add .`, `git commit -m \"Автоматическое обновление результатов для модуля <module_id>\"`, `git push origin <ветка>`.\
        - Использовать учетные данные (токен/ключ) из секретов Coolify.
...
- **Инфраструктура:** Docker, Coolify (Node.js App, PostgreSQL/Supabase, Qdrant).
- **Внешние API:** Git (Docs Repo, App Repo), OpenRouter, vsegpt.ru.
- **Переменные Окружения / Секреты:**
    - Все предыдущие (`SOURCE_MD_ROOT_DIR`, `MIN/MAX_CHUNK...`, `OPENROUTER_API_KEY`, `GIT_REPO_URL` (Docs), `GIT_SSH_KEY_PATH`/`GIT_ACCESS_TOKEN` (Docs), `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `QDRANT_URL`, `QDRANT_API_KEY`, `VSEGPT_API_KEY`).
    - **Новые:**
        - `WEBHOOK_SECRET`: Секрет для валидации входящих webhook\'ов (использовать **Secret**).
        - `GIT_APP_REPO_URL`: URL Git-репозитория **Приложения**, куда пушить результаты.
        - `GIT_RESULTS_PUSH_TOKEN` (или `GIT_RESULTS_SSH_KEY_PATH`): Токен или путь к ключу для `git push` в репозиторий Приложения (использовать **Secret**).
...
- **Развертывание на Coolify:**
    - Развернуть Node.js приложение, Supabase, Qdrant.
    - Настроить **все** переменные окружения и **секреты**, включая новые для webhook\'а и пуша результатов.
...
- **Передача Заказчику:**
    ...
    - `README.md` должен содержать инструкции по:
        ...
        - Получению API-ключей и **токена/ключа для пуша результатов**.
        - Заполнению `.env.example` / секретов Coolify.
        ...
        - **Описанию структуры папки с результатами** в репозитории Приложения.
...
- Структурированные данные сохраняются в Supabase...
- Данные успешно векторизуются и загружаются в Qdrant...
- Данные разных модулей логически изолированы.
- **Результаты обработки (чанковые `.md` файлы) успешно коммитятся и пушатся в репозиторий Приложения.**
...
- **v1.4 (23.04.2025):** Переход на автоматический запуск конвейера через Git webhook. Добавлены требования к HTTP-эндпоинту, обработке webhook\'а, асинхронному запуску и пушу результатов в Git-репозиторий приложения. Обновлена архитектура, диаграмма, технические требования, инструкции по развертыванию и критерии успеха.

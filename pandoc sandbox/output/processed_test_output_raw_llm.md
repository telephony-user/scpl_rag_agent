...
sequenceDiagram
    ...
    AgentHelper->>JSSDK: 6 agentOpenConnect
    JSSDK->>AgentHelper: 7 resultAgentOpenConnect
    AgentHelper->>AgentServer: 8 организация wss подключения к AgentServer с учетом retry подключения
    AgentServer->>Auth: 9 проверка валидности полученного токена
    Auth-->>AgentServer: 10 результат проверки
    AgentServer-->>JSSDK: 11 agentHelperConnectionState
```
